...
const NftItem: FunctionComponent<NftItemProps> = ({ item, buyNft }) => {
  ...
  return (
    <>
      <div className="flex-shrink-0 h-60 overflow-hidden justify-center flex bg-slate-200 items-center">
        <Image
      ...
      <div className="flex-1 bg-white p-6 flex flex-col justify-between">
        <div className="flex-1">
          ...
          <div className="block mt-2">
            <p className="text-xl font-semibold text-gray-900 flex justify-between">
              <span>{item.meta.name}</span><span>{item?.meta?.type}</span>
            </p>
        ...
        <div className="overflow-hidden mb-4">
          <dl className="-mx-4 -mt-4 flex flex-wrap">
            ...
            {item.meta.custorm?.map((attribute: any) => {
              return (<div key={attribute.name} className="flex flex-col px-4 pt-4">
        ...
        <div>
          <button
            onClick={() => {
              // erc20Adress: string, owner: string, userList: string[], priceList: number[]
              buyNft(item.tokenId, item.price, ERC20_Token_Address, ERC20_AIRDROP_TOKEN, [account.data] as string[], [30]);
            }}
            type="button"
